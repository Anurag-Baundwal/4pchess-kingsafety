...
export class Board {
    constructor(turn, location_to_piece, castling_rights = null, lives = null) {
        this.turn = turn;
    ...
    makeMove(move) {
        ...
        //var opponent = getNextPlayer(this.turn);
        //if (this.isAttackedByTeam(opponent.getTeam(), this.getKingLocation(opponent))) {
        //    this.lives[opponent.getColor()]--;
        //}

        if (this.isAttackedByTeam(Team.BlueGreen, this.getKingLocation(kRedPlayer))) {
            this.lives[kRedPlayer.getColor()]--;
        }
        if (this.isAttackedByTeam(Team.RedYellow, this.getKingLocation(kBluePlayer))) {
            this.lives[kBluePlayer.getColor()]--;
        }
        if (this.isAttackedByTeam(Team.BlueGreen, this.getKingLocation(kYellowPlayer))) {
            this.lives[kYellowPlayer.getColor()]--;
        }
        if (this.isAttackedByTeam(Team.RedYellow, this.getKingLocation(kGreenPlayer))) {
            this.lives[kGreenPlayer.getColor()]--;
        }
    ...
    getAllLegalMoves() {
        ...
        for (var placed_piece of [...placed_pieces]) {
            ...
            for (var move of piece_moves) {
                this.makeMove(move);
                if (this.lives[this.turn.getColor()] > 0) {
    ...
    static CreateStandardSetup() {
        ...
        player_colors.forEach((color, index) => {
            ...
            if (color == RED) {
                piece_location = new BoardLocation(7, 5);
                location_to_piece[piece_location] = [piece_location, new Piece(player, KING)];
            }
            if (color == BLUE) {
                piece_location = new BoardLocation(5, 6);
                location_to_piece[piece_location] = [piece_location, new Piece(player, KING)];
            }
            if (color == YELLOW) {
                piece_location = new BoardLocation(6, 8);
                location_to_piece[piece_location] = [piece_location, new Piece(player, KING)];
            }
            if (color == GREEN) {
                piece_location = new BoardLocation(8, 7);
                location_to_piece[piece_location] = [piece_location, new Piece(player, KING)];
            }
