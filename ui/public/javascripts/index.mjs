...
(function () {
    ...
    function displayBoard() {
        ...
        if (eval_results != null && 'evaluation' in eval_results) {
            ...
            // var lives_info = `lives: red ${board.GetLives[0]}, blue ${board.lives[PlayerColor.Blue]}, yellow ${board.lives[PlayerColor.Yellow]}, green ${board.lives[PlayerColor.Green]}`;
            var lives_info = `lives: red ${board.GetLives(board_util.PlayerColor.Red)}, blue ${board.GetLives(board_util.PlayerColor.Blue)}, yellow ${board.GetLives(board_util.PlayerColor.Yellow)}, green ${board.GetLives(board_util.PlayerColor.Green) }`;

            $('#eval_estimate').html(eval_html);
            $('#turn_info').html(turn_info);
            $('#lives_info').html(lives_info);
    ...
    function getBoardState() {
        ...
        state['board'] = [];
        state['lives'] = {
            'red': board.GetLives(board_util.PlayerColor.Red),
            'blue': board.GetLives(board_util.PlayerColor.Blue),
            'yellow': board.GetLives(board_util.PlayerColor.Yellow),
            'green': board.GetLives(board_util.PlayerColor.Green),
        };
